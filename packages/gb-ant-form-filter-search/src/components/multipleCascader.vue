<template>
  <div
    class="filter-sub-multiple-cascader"
    :class="{
      'filter-sub-multiple-cascader-outside': !isClickOutSide,
      'hide-afterline': hideAfterLine
    }"
    v-clickoutside="handleDropDownOutSide"
  >
    <div class="filter-sub-multiple-cascader-item">
      <span v-show="value && value.length > 0" class="label-text">{{
        placeholder
      }}</span>
      <a-select
        v-bind="$attrs"
        ...
        :class="{
          ...
          'no-data': !selectedValueArr || selectedValueArr.length <= 0
        }"
        ...
        :disabled="!selectedValueArr || selectedValueArr.length <= 0"
        :open="isOpen"
        showArrow
      ...
      >
        ...
        <div
          slot="suffixIcon"
          class="icon-down-no-data"
          @click.stop="handleIconDropDown"
          v-if="!selectedValueArr || selectedValueArr.length <= 0"
        >
        ...
        <template slot="dropdownRender" slot-scope="menu">
          ...
          <div :class="dropdownClassName">
            ...
            <div v-else class="cascader-content-wrap">
              <div class="cascader-content-container">
                ...
                >
                  <li
                    ...
                    @click="handleCascaderAllItemClick"
                    v-if="levelIndex === 0"
                  >
                    <a-checkbox
        ...
        <a-select-option
          ...
          :label="item[selectOptionsConfig.text]"
        >
...
export default {
  ...
  methods: {
    handleIconDropDown(e) {
      e.stopPropagation();
      e.cancelBubble = true;
      this.isClickOutSide = this.isOpen;
...
.filter-sub-multiple-cascader {
  ...
  &.filter-sub-multiple-cascader-outside {
    .ant-select-arrow {
      .icon-down {
        transform: rotate(0);
      }
...
.filter-sub-multiple-cascader {
  ...
  .filter-sub-multiple-cascader-item {
    position: relative;
    display: flex;
    flex-direction: row;
    align-items: center;
